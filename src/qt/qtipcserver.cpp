...
static bool ipcScanCmd(int argc, char *argv[], bool fRelay)
{
    ...
    for (int i = 1; i < argc; i++)
    {
        if (boost::algorithm::istarts_with(argv[i], "bitcoin:"))
        {
            ...
            try {
                boost::interprocess::message_queue mq(boost::interprocess::open_only, BPUMPOINURI_QUEUE_NAME);
...
static void ipcThread2(void* pArg)
{
    ...
    message_queue::remove(BPUMPOINURI_QUEUE_NAME);
...
void ipcInit(int argc, char *argv[])
{
    ...
    try {
        mq = new message_queue(open_or_create, BPUMPOINURI_QUEUE_NAME, 2, MAX_URI_LENGTH);
