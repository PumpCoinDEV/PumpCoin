...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // PumpCoin: starting difficulty is 1 / 2^12
...
{
    // PumpCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 nSubsidy = 1000 * COIN;
	...
	// Susidy is cut in half every 86400 (43200=1month) block, Which is about every 60 days.
	nSubsidy >>= (nHeight / 86400); 

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 24 * 60 * 60; // PumpCoin: 1 day
static const int64 nTargetSpacing =  60; // PumpCoin: 1 Minute
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // PumpCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, PumpCoin - DarkGravity v2, written by Evan Duffield - evan@PumpCoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // PumpCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // PumpCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x0000034200104794a820a5b35c1ca49708be0617279d2df0ccab3adef4c07bd6");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nVersion = 1;
        block.nTime    = 1397916000; // 19 Aprile - 16:00
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 592149;

        if (fTestNet)
        {
            block.nTime    = 1397916000;
            block.nNonce   = 998757;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x1a3e0752699fe1921d91a60233664db04aee88202e947fec0551b1f1c11b14a6"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf6, 0xc6, 0xb6, 0xd6 }; // PumpCoin: increase each by adding 2 to bitcoin's value.
...
void static PumpCoinMiner(CWallet *pwallet)
{
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PumpCoinMiner, pwallet));
}
